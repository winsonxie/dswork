...
$dswork.ztree.dataFilter = function (treeId, parentNode, data){
	var d=[];if(data){for(var i = 0;i < data.length;i++){
		if(data[i].status == 2){data[i].iconSkin = "groups";d.push(data[i]);}
		else if(data[i].status == 1){data[i].iconSkin = "group";d.push(data[i]);}
	}}return d;
};
</script>
</head>
