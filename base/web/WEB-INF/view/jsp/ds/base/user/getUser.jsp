...
$dswork.page.join = function(td, menu, id){
	...
	$(menu).append($('<div iconCls="menuTool-update">重置密码</div>').bind("click", function(){
		location.href = "updUserPassword1.htm?xtype=${fn:escapeXml(param.xtype)}&type=${fn:escapeXml(param.type)}&page=${pageModel.page}&keyIndex=" + id;
	}));
	$(menu).append($('<div iconCls="menuTool-update">修改</div>').bind("click", function(){
		location.href = "updUser1.htm?xtype=${fn:escapeXml(param.xtype)}&type=${fn:escapeXml(param.type)}&page=${pageModel.page}&keyIndex=" + id;
	}));
	}
	$(menu).append($('<div iconCls="menuTool-user">调动</div>').bind("click", function(){
		location.href = "updUserOrg1.htm?xtype=${fn:escapeXml(param.xtype)}&type=${fn:escapeXml(param.type)}&page=${pageModel.page}&keyIndex=" + id;
	}));
};
$(function(){
	...
	<tr>
		...
		<td class="menuTool">
			...
			<c:if test="${'root'!=d.account && 'admin'!=d.account}">
				<a class="update" href="updUser1.htm?xtype=${fn:escapeXml(param.xtype)}&type=${fn:escapeXml(param.type)}&page=${pageModel.page}&keyIndex=${d.id}">修改</a>
			</c:if>
			<a class="user" href="updUserOrg1.htm?xtype=${fn:escapeXml(param.xtype)}&type=${fn:escapeXml(param.type)}&page=${pageModel.page}&keyIndex=${d.id}">调动</a>
