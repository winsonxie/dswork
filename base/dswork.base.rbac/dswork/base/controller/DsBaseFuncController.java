...
import dswork.base.model.DsBaseRes;
import dswork.base.model.DsBaseSystem;
import dswork.base.model.view.DsBaseFuncView;
import dswork.base.service.DsBaseFuncService;
import dswork.core.util.CollectionUtil;
import dswork.mvc.BaseController;

// 功能
@Controller
@RequestMapping("/ds/base/func")
public class DsBaseFuncController extends BaseController
{
	@Autowired
	private DsBaseFuncService service;
	...
	public String addFunc1()
	{
		...
		{
			parent = new DsBaseFunc();
		}
		put("parent", parent);
		put("systemid", systemid);
		put("pid", req().getLong("pid"));
		return "/ds/base/func/addFunc.jsp";
	}

	@RequestMapping("/addFunc2")
	public void addFunc2(DsBaseFunc po)
	{
		try
		{
			...
			if(0 < po.getPid())
			{
				DsBaseFunc parent = service.get(po.getPid());
				if(null == parent)
				{
					print("0:参数错误，请刷新重试");
					return;
	...
	public String updFunc1()
	{
		...
		List<DsBaseRes> list = po.getResourcesList();
		put("po", po);
		put("parent", parent);
		put("list", list);
		put("count", count);
		return "/ds/base/func/updFunc.jsp";
	}

	@RequestMapping("/updFunc2")
	public void updFunc2(DsBaseFunc po)
	{
		try
		{
			...
			DsBaseFunc _po = service.get(po.getId());
			if(null == _po)
			{
				print("0:操作失败，请刷新重试");
				return;
	...
	public String updFuncSeq1()
	{
		long systemid = req().getLong("systemid");
		long pid = req().getLong("pid");
		List<DsBaseFunc> list = service.queryList(systemid, pid);
		put("list", list);
		return "/ds/base/func/updFuncSeq.jsp";
	}
	...
	public String updFuncMove1()
	{
		...
		put("po", po);
		return "/ds/base/func/updFuncMove.jsp";
	}
	...
	public String getFuncTree()
	{
		long systemid = req().getLong("systemid");
		put("systemid", systemid);
		return "/ds/base/func/getFuncTree.jsp";
	}
	...
	public String getFunc()
	{
		...
		put("systemid", systemid);
		put("pid", pid);
		return "/ds/base/func/getFunc.jsp";
	}
	...
	public String getFuncById()
	{
		...
		put("po", po);
		put("list", list);
		return "/ds/base/func/getFuncById.jsp";
	}
	...
	{
		long systemid = req().getLong("systemid");
		List<DsBaseFuncView> list = service.queryFuncBySystemid(systemid);
		Map<String, DsBaseFuncView> map = new HashMap<String, DsBaseFuncView>();
		List<DsBaseFuncView> rootList = new ArrayList<DsBaseFuncView>();
		for(DsBaseFuncView m : list)
		{
			m.setIndex(0);
			map.put(m.getId() + "", m);
			if(m.getPid() == 0)
		...
		return "/ds/base/func/getFuncEdit.jsp";
	}
	...
	public void getFuncEdit2()
	{
		try
		{
			...
			List<DsBaseFunc> list = new ArrayList<DsBaseFunc>();
	 ...
	 * @param seq 排序号
	 */
	private static boolean doFuncList(List<DsBaseFunc> list, List<String> idList, String jsonStr, long pid, long systemid)
	{
		// Json的解析类对象
