...
(
  ID                   BIGINT(18)    COMMENT '分类ID' NOT NULL,
...
(
  ...
  CATEGORYID           BIGINT(18)    COMMENT '分类' NOT NULL,
  ALIAS                VARCHAR(64)   COMMENT '流程标识' NOT NULL,
  VNUM                 INT(10)       COMMENT '内部版本0为编辑版本' NOT NULL,
...
(
  ID                   BIGINT(18)    COMMENT '主键ID' NOT NULL,
  FLOWID               BIGINT(18)    COMMENT '流程ID' NOT NULL,
  DEPLOYID             VARCHAR(128)  COMMENT '流程发布ID，当前版本此值为空',
  TALIAS               VARCHAR(64)   COMMENT '节点标识(start开始，end结束)' NOT NULL,
  TNAME                VARCHAR(30)   COMMENT '节点名称',
  TCOUNT               INT(4)        COMMENT '合并任务个数(只有一个任务时等于1，其余大于1)' NOT NULL,
  TNEXT                VARCHAR(1000) COMMENT '下级任务(以逗号分隔节点标识，以|线分隔分支任务)',
  TUSERS               VARCHAR(1000) COMMENT '当前任务的用户ID(以逗号分隔可选用户，以|线分隔分支任务用户)',
  SUBCOUNT             INT(4)        COMMENT '至少合并会签个数(不需要会签时值为-1)' NOT NULL,
...
(
  ...
  DEPLOYID             VARCHAR(128)  COMMENT '流程发布ID' NOT NULL,
  PIDAY                INT(10)       COMMENT '时限天数',
...
(
  ID                   BIGINT(18)    COMMENT '主键ID' NOT NULL,
  PIID                 BIGINT(18)    COMMENT '实例ID' NOT NULL,
  YWLSH                VARCHAR(128)  COMMENT '业务流水号',
  SBLSH                VARCHAR(128)  COMMENT '申办流水号',
  FLOWID               BIGINT(18)    COMMENT '流程ID' NOT NULL,
  FLOWNAME             VARCHAR(30)   COMMENT '流程名称',
  TALIAS               VARCHAR(64)   COMMENT '任务标识' NOT NULL,
  TNAME                VARCHAR(300)  COMMENT '任务名称',
  TCOUNT               INT(10)       COMMENT '合并任务个数(只有一个任务时等于1，其余大于1)',
  TPREV                VARCHAR(64)   COMMENT '上级任务(从哪过来的)',
  TNEXT                VARCHAR(1000) COMMENT '下级任务(以逗号分隔节点标识，以|线分隔分支任务)',
  ...
  TMEMO                VARCHAR(1000) COMMENT '参数',
  DATATABLE            TEXT          COMMENT '数据结构',
  TENABLE              INT(1)        COMMENT '是否启用待办（-1不启用，0启用）',
  DATAVIEW             TEXT          COMMENT '数据视图',
  PRIMARY KEY (ID),
  CONSTRAINT FK_DS_FLOW_PI_WAITING FOREIGN KEY (PIID)
     REFERENCES DS_FLOW_PI (ID) ON DELETE CASCADE ON UPDATE CASCADE
) COMMENT '流程待办事项';
