package dswork.flow.controller;
...
import dswork.flow.model.DsFlowDataRow;
import dswork.flow.model.DsFlowTask;
import dswork.flow.service.DsBaseFlowService;
...
public class DsBaseFlowController extends BaseController
{
	...
	public void addFlow2(DsFlow po)
	{
		try
		{
	...
	public String updFlow1()
	{
		...
		{
			DsFlowTask task = taskList.get(i);
	...
	public void updFlow2(DsFlow po)
	{
		try
		{
			Map<String, String> map = new LinkedHashMap<String, String>();
	...
	public void addFlowCategory2(DsFlowCategory po)
	{
		try
		{
			...
			{
				DsFlowCategory parent = service.get(po.getPid());
				if (null == parent)
				{
					print("0:参数错误，请刷新重试");
					return;
	...
	public void updFlowCategory2(DsFlowCategory po)
	{
		try
		{
			...
			DsFlowCategory old = service.get(po.getId());
			if (null == old)
			{
				print("0:操作失败，请刷新重试");
				return;
			...
			{
				DsFlowCategory parent = service.get(old.getPid());
				if (null == parent)
				{
					print("0:参数错误，请刷新重试");
					return;
	...
	public void updFlowCategoryMove2()
	{
		...
		{
			DsFlowCategory m = service.get(pid);
			if (m == null)
			{
