...
import dswork.base.model.DsBaseFlowDataRow;
import dswork.base.model.DsBaseFlowTask;
import dswork.base.service.DsBaseFlowService;
...
public class DsBaseFlowController extends BaseController
{
	...
	public void addFlow2(DsBaseFlow po)
	{
		try
		{
			...
			{
				DsBaseFlowCategory fc = service.get(po.getCategoryid());
				...
				if (fc != null)
				{
					...
					{
						List<DsBaseFlowTask> taskList = new ArrayList<DsBaseFlowTask>();
						if (po.getFlowxml().length() < 50)
						{
							...
							{
								DsBaseFlowTask m = new DsBaseFlowTask();
	...
	public String updFlow1()
	{
		...
		DsBaseFlow po = service.getFlow(id);
		Map<String, String> taskMap = new HashMap<String, String>();
		List<DsBaseFlowTask> taskList = po.getTaskList();
		for (int i = 0; i < taskList.size(); i++)
		{
			DsBaseFlowTask task = taskList.get(i);
		...
		put("po", po);
		put("taskMap", taskMap);
		return "/ds/base/flow/updFlowXML.jsp";
	}

	@RequestMapping("/updFlow2")
	public void updFlow2(DsBaseFlow po)
	{
		try
		{
			Map<String, String> map = new LinkedHashMap<String, String>();
	...
	public String getFlow()
	{
		...
		if (categoryid > 0)
		{
			DsBaseFlowCategory po = service.get(categoryid);
			if (po != null)
			{
				...
				return "/ds/base/flow/getFlow.jsp";
			}
		}
		return null;
	}
	...
	public String addFlowCategory1()
	{
		...
		DsBaseFlowCategory parent = null;
		if (pid > 0)
		{
		...
		{
			parent = new DsBaseFlowCategory();
		}
		put("parent", parent);
		put("pid", pid);
		return "/ds/base/flow/addFlowCategory.jsp";
	}

	@RequestMapping("/addFlowCategory2")
	public void addFlowCategory2(DsBaseFlowCategory po)
	{
		try
		{
			...
			{
				DsBaseFlowCategory parent = service.get(po.getPid());
				if (null == parent)
				{
					print("0:参数错误，请刷新重试");
					return;
	...
	{
		Long id = req.getLong("keyIndex");
		DsBaseFlowCategory po = service.get(id);
		if (null == po)
		{
		...
		put("po", po);
		put("parent", parent);
		return "/ds/base/flow/updFlowCategory.jsp";
	}

	@RequestMapping("/updFlowCategory2")
	public void updFlowCategory2(DsBaseFlowCategory po)
	{
		try
		{
			...
			DsBaseFlowCategory old = service.get(po.getId());
			if (null == old)
			{
				print("0:操作失败，请刷新重试");
				return;
			...
			{
				DsBaseFlowCategory parent = service.get(old.getPid());
				if (null == parent)
				{
					print("0:参数错误，请刷新重试");
					return;
	...
	{
		long pid = req.getLong("pid");
		List<DsBaseFlowCategory> list = service.queryList(pid);
		put("list", list);
	...
	public void updFlowCategoryMove2()
	{
		...
		{
			DsBaseFlowCategory m = service.get(pid);
			if (m == null)
			{
	...
	public String getFlowCategoryTree()
	{
		...
		{
			po = new DsBaseFlowCategory();
		}
		put("po", po);
		return "/ds/base/flow/getFlowCategoryTree.jsp";
	}
	...
	public String getFlowCategory()
	{
		...
		put("rootid", rootid);
		put("pid", pid);
		return "/ds/base/flow/getFlowCategory.jsp";
	}
